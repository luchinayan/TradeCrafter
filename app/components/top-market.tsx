...
export default function TopMarket() {
  ...
  return (
    <div className="bg-neutral-s-4 mb-16 flex justify-around items-center rounded-3xl mx-auto max-w-7xl">
      {data &&
        data.slice(0, 4).map((val: any) => {
          return (
            <Link
              href={"/exchange"}
              className="p-8 my-4 hover:bg-white rounded-3xl"
              key={val.id}>
              <Image
                width={50}
                height={50}
                ...
                alt={"topmarket"}></Image>
              <div className="py-4">
                <div className="text-s text-neutral-s-1">
                  {val.symbol}/USDT
                  <div
                    className={`ml-5 rounded-3xl inline text-white px-2 text-s font-light
                        ${
